...
# 生成 /etc/systemd/system/rosetta.service 并 enable --now。
# 前置：1) 已编辑 script/env（HARNESS_PASSWORD）2) 已完成 pnpm install && pnpm build
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SERVICE_NAME="rosetta"
...
# 由 rosetta/script/install-systemd.sh 生成，可重跑覆盖
[Unit]
Description=rosetta harness (pi agent web harness)
...
Restart=on-failure
RestartSec=3
# 日志进 journald: journalctl -u rosetta -f
...
echo "   systemctl status rosetta      # 状态"
echo "   sudo systemctl restart rosetta"
echo "   journalctl -u rosetta -f      # 跟日志"
