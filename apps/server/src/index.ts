...
import { runMigrations, sqlite, db } from "./db/index.ts";
...
async function main(): Promise<void> {
  ...
  const finishing = db.select().from(tasks).where(eq(tasks.status, "finishing")).all();
  if (finishing.length) {
    db.update(tasks)
      .set({ status: "failed", error: "server 重启时 git 流水线被中断，请 retry" })
      .where(eq(tasks.status, "finishing"))
      .run();
    for (const t of finishing) {
      if (t.worktreePath) {
        db.update(worktrees).set({ status: "idle", updatedAt: Date.now() }).where(eq(worktrees.path, t.worktreePath)).run();
      }
    }
  }

  await backfillSessions().catch((e) => log.error("[backfill] 失败（忽略）:", e));
  ...
  log.info(`rossetta server 已启动: http://${config.host}:${config.port}`);
