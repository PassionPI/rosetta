import type { ProjectDTO, SessionSummary } from "@rosetta/shared";
  ...
  return (
    <div className="page">
      <div className="card">
        ...
        <div className="row">
          <input
            placeholder="项目路径（worktree 绝对路径）"
            value={newCwd}
          ...
          />
          <button
            disabled={!newCwd.trim() || create.isPending}
      ...
      {projects.data && projects.data.length > 0 && (
        <div className="chips">
          {projects.data.map((p) => (
            <button
              key={p.path}
              className={`chip ${cwd === p.path ? "active" : ""}`}
      ...
      {grouped.length === 0 && (
        <div className="muted">暂无会话。上面输入项目路径创建第一个。</div>
      )}
      ...
      {grouped.map(([groupCwd, list]) => (
        <section key={groupCwd}>
          ...
          {list.map((s) => (
            <a
              key={s.id}
              className={`item ${s.status === "active" ? "live" : ""}`}
              href={`#/session/${s.id}`}
            >
              <span className="title">{s.name || s.id.slice(0, 8)}</span>
              <span className="meta">
                {s.taskId && (
                  <span className="badge task">task #{s.taskId}</span>
                )}
